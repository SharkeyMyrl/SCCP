...
namespace LandSharks.Commands
{
    internal class CommandVault : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            if (!SharkTank.Config.Enabled)
            {
                return;
            }
            VaultPlayerComponent component = player.Player.transform.GetComponent<VaultPlayerComponent>();
            ...
            Logger.Log("Vault size taken: " + size);
#endif
            inventory.resize(8, size);

            List<ItemJar> items = VaultDatabase.RetrieveItems(player.CSteamID);
            ...
            {
                VaultDatabase.AddItem(player.CSteamID, jar);
            };

            inventory.onItemRemoved = (byte page, byte index, ItemJar jar) =>
            {
                VaultDatabase.DeleteItem(player.CSteamID, jar);
            };


            inventory.onItemUpdated = (byte page, byte index, ItemJar jar) =>
            {
                VaultDatabase.UpdateItem(player.CSteamID, jar);
            };
